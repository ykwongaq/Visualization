...
def main(args):
    ...
    assert len(input_dirs) == len(
        captions
    ), f"Number of captions do not match the number of folders. Expected {len(input_dirs)} captions, got {len(captions)}"
    ...
    assert num_rows * num_cols >= len(
        input_dirs
    ), f"Number of rows and columns should be greater than the number of folders. Expected {len(input_dirs)} <= {num_rows*num_cols}"
    ...
    for images_files in images_files_list:
        assert (
            len(images_files) == num_images
        ), "Number of images in the folders do not match"
...
if __name__ == "__main__":
    print(
        "Make sure that the corresponding depth maps have the same name or are in the same order after sorting by name"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help="List of directories containing depth maps",
    )
    parser.add_argument(
        "--output_dir", type=str, help="Output directory to save the visualization"
    )
    parser.add_argument(
        "--num_rows", type=int, default=1, help="Number of rows in the visualization"
    )
    parser.add_argument(
        "--num_cols", type=int, default=1, help="Number of columns in the visualization"
    )
    parser.add_argument(
        "--captions", nargs="+", type=str, help="List of captions for each depth map"
    )
